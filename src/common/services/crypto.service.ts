...
import { ResponseCodes, SupportedCryptoCurrencies, SupportedCurrencies } from "../enums";
import Coinbase, { Client, CreateCharge, Webhook } from 'coinbase-commerce-node'
...
export class CryptoPaymentService {
    ...
    async createCheckoutSession(payload: {
        ...
        const currency = [SupportedCryptoCurrencies.ERC20, SupportedCryptoCurrencies.TRC20].includes(payload.currency as SupportedCryptoCurrencies) ? 'USDT' : payload.currency
        ...
        const options = {
            ...
            body: JSON.stringify({
                itemizedDetails: { isFee: false, amount: payload.totalAmount, description: `Car rental of car: ${payload.lineItems[0]?.productName}` },
                buyer: {
        ...
        return (await this.httpService.post<any>(`${this.API.baseUrl}/invoices`, options))?.data
    ...
    async confirmPayment(transactionId: string) {
        const invoice = await this.httpService.get<any>(`${this.API.baseUrl}/invoices/${transactionId}?token=${this.API.apiKey}`, {
            headers: {
                accept: 'application/json',
                'Content-Type': 'application/json',
                'X-Accept-Version': '2.0.0'
    ...
    async cancelPayment(transactionId: string) {
        ...
        const cancelledInvoice = await this.httpService.delete<any>(`${this.API.baseUrl}/invoices/${transactionId}?token=${this.API.apiKey}`, {
            headers: {
                accept: 'application/json',
                'Content-Type': 'application/json',
                'X-Accept-Version': '2.0.0'
    ...
    async processRefund(transactionId: string) {
        ...
        const refunded = await this.httpService.post<any>(`${this.API.baseUrl}/refunds`, {
            headers: {
                accept: 'application/json',
                'Content-Type': 'application/json',
                'X-Accept-Version': '2.0.0'
