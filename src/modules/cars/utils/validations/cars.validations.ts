...
const carDocumentValidator = Joi.object<CarDocument>({
  ...
  type: Joi.string()
    .valid(CarDocumentType.ACCIDENT, ...Object.values(CarDocumentType))
    .required(),
...
const carOwnerValidator = Joi.object<CarOwnerDetail>({
  ...
  status: Joi.string()
    .valid('Active', 'Pending', 'Transferred')
    .optional()
    .allow(null),
...
export const CreateCarSchema = Joi.object<CreateCarDTO>({
  acquisitionDate: Joi.string()
    .optional()
    .custom((val) => !isNaN(new Date(val).getTime()), 'must be a valid date')
    .allow(null)
    .allow(''),
  blockchainId: Joi.string().optional().allow('').allow(null),
  category: Joi.string()
    .valid(CarCategory.CONVERTIBLE, ...Object.values(CarCategory))
    .required(),
  condition: Joi.string()
    .valid(CarCondition.EXCELLENT, ...Object.values(CarCondition))
    .required(),
  currentStatus: Joi.string()
    .valid(CarStatus.AVAILABLE, ...Object.values(CarStatus))
    .optional()
    .allow(null),
  dimensions: carDimentionValidator.required(),
  documents: Joi.array()
    .required()
    .items(carDocumentValidator)
    .optional()
    .allow(null),
  ...
  fuelType: Joi.string()
    .valid(FuelType.DIESEL, ...Object.values(FuelType))
    .required(),
  inspectionStatus: Joi.string()
    .valid(CarInspectionStatus.EXEMPTED, ...Object.values(CarInspectionStatus))
    .optional()
    .allow(null),
  lastInspectionDate: Joi.date().optional().allow('').allow(null),
  listingType: Joi.string()
    .valid(CarListingType.FOR_RENT, ...Object.values(CarListingType))
    .optional()
    .allow('')
    .allow(null),
  ...
  transmission: Joi.string()
    .valid(TransmissionType.AUTOMATIC, ...Object.values(TransmissionType))
    .required(),
