...
export class CarFeatureService {
    ...
    async getFeature(
        ...
        const serialised = this.serializer.serialize(CarFeatureDto, feature, locale);
    ...
    async validateSlug(name: string) {
        const found = await this.featureRepository.getFeatureBySlug(slugify(name), 'en')
    ...
    async listFeatures(
        ...
        const serialised = this.serializer.serialize(CarFeatureDto,
            features,
            rest?.locale || 'en'
        );
    ...
    createFeature(data: CreateFeatureDTO) {
        return new CreateFeatureUseCase(this.featureRepository, this.translationsRepo, this.serializer, this.messageBroker).execute(data)
    }

    updateFeature(id: string, data: UpdateFeatureDTO, actor: User) {
        return new UpdateCarFeatureUseCase(this.featureRepository, this.messageBroker, this.serializer,).execute(id, data, actor)
    }

    deleteFeatures(id: string | string[], actor: User, hardDelete: boolean = false) {
        return new DeleteCarFeatureUseCase(this.featureRepository, this.messageBroker).execute(id, actor, hardDelete)
    }
}
