...
export class CarService {
  ...
  async validateSlug(name: string) {
    const found = await this.carsRepository.getCarBySlug(slugify(name), {
      locale: 'en',
    });
  ...
  updateCar(
    ...
    return new UpdateCarUseCase(
      this.carsRepository,
      this.messageBroker,
      this.serializer
    ).execute(carId, data, actor);
  }

  deleteCars(carId: string | string[], actor: User, hardDelete?: boolean) {
    return new DeleteCarUseCase(
      this.carsRepository,
      this.messageBroker
    ).execute(carId, actor, hardDelete);
  }
}
