...
import { modelUpdated } from '../../../utils/messages.json'
import { CarBrandRepository } from '@/modules/cars/infrastrucure/brand.repository';

export class UpdateCarModelUseCase implements IUseCase<[string, UpdateModelDTO, User], IReturnValue<CarModelDto>> {
  ...
  async execute(
    ...
    // Ensure Model exists
    const foundModel = await this.modelRepository.findOne({ where: { id }, relations: ['brand', 'translations'] }) // we want to load all the existing translations
    ...
    if (enTranslation) {
      const found = await this.modelRepository.findOneBy({ id: Not(id), slug: slugify(enTranslation.name) })
    ...
    const updatedModel = await this.modelRepository.manager.transaction(async (manager) => {
      ...
      if (translations && translations?.length) {
        const existingTranslationsMap = new Map(foundModel?.translations?.map(itm => [itm.locale, itm]))
    ...
    const serialisedModel = this.serializer.serialize(CarModelDto, updatedModel, enTranslation?.locale)

    try {
      this.messageBus.publishMessage(modelUpdated, {
        data: {
      ...
      logger.error(`Failed to pubish message ${modelUpdated}`, error)
    }
