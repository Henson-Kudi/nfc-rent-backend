...
export class CreateModelUseCase
  implements IUseCase<[CreateModelDTO], IReturnValue<CarModelDto>>
{
  constructor(
  ...
  async execute(data: CreateModelDTO): Promise<IReturnValue<CarModelDto>> {
    ...
    const savedModel = await this.modelRepository.manager.transaction(
      async (manager) => {
    ...
    const serializedModel = this.serializer.serialize(
      CarModelDto,
      savedModel,
    ...
    try {
      this.messageBroker.publishMessage(FleetEvents.model.created, {
        data: serializedModel,
      });
    } catch (error) {
      logger.error(
        `Failed to pubish message ${FleetEvents.model.created}`,
        error
      );
    }
