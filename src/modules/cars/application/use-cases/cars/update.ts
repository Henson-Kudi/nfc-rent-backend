import { Car, CarDocument, CarFeature, CarMedia, CarModel, CarOwnershipDetail, CarTranslation, RentalPricing, User } from "@/common/entities";
import { CarCategory, CarCondition, CarDocumentType, CarInspectionStatus, CarListingType, CarStatus, FuelType, ResponseCodes, TransmissionType } from "@/common/enums";
...
import { carUpdated } from '../../../utils/messages.json'

export class UpdateCarUseCase implements IUseCase<[string, UpdateCarDTO, User], IReturnValue<CarDto>> {
    ...
    async execute(carId: string, data: UpdateCarDTO, actor: User): Promise<IReturnValue<CarDto>> {
        // Validate the data
        ...
        const enTranslation = valid?.translations?.find(itm => itm?.locale === 'en')
        ...
        const { category, fuelType, transmission, currentStatus, listingType, acquisitionDate, condition, inspectionStatus, lastInspectionDate, nextInspectionDueDate, media, model, features, rentalPricings, documents, owner, translations, ...rest } = valid

        const updatedCar = await this.carRepo.manager.transaction(async (manager) => {
            ...
            if (media && media?.length) {
                ...
                // save media
                const newMedia = await repo.save(media.map(itm => repo.create({
                    ...itm,
                    car: foundCar
                })))
            ...
            if (rentalPricings && rentalPricings?.length) {
                ...
                // save prices
                const saved = await repo.save(rentalPricings.map(itm => repo.create({
            ...
            if (documents && documents?.length) {
                ...
                // save documents
                const saved = await repo.save(documents.map(itm => repo.create({
                    ...itm,
            ...
            if (owner) {
                ...
                const existingOwner = await ownersRepo.findOne({
                    where: {
                        car: foundCar,
                        owner: foundUser
                    }
                ...
                existingOwner ? await ownersRepo.update(existingOwner.id, {
                    ...existingOwner,
                    ...owner,
                    acquiredDate: owner?.acquiredDate ? new Date(owner.acquiredDate) : existingOwner?.acquiredDate || undefined,
                    transferDate: owner?.transferDate ? new Date(owner.transferDate) : existingOwner?.transferDate || undefined,
                }) : await ownersRepo.save(ownersRepo.create({
                    ...owner,
                    owner: foundUser,
                    car: foundCar,
                }))
            ...
            if (translations && translations?.length) {
                ...
                const existingTranslationMap = new Map((foundCar?.translations || [])?.map(item => [item.locale, item]))
                ...
                const trans = translations.map(itm => {
                    ...
                    return existingTranslation
                        ...
                        : repo.create({ ...itm, parent: foundCar, parentId: foundCar.id });
                })
            ...
            const listingTypes: CarListingType[] = listingType ? [...new Set(...[...foundCar.listingType, listingType]).values()] as CarListingType[] : foundCar.listingType
            ...
            const mergedCar = repo.merge(foundCar, {
                ...
                inspectionStatus: inspectionStatus as CarInspectionStatus || undefined,
                lastInspectionDate: lastInspectionDate ? new Date(lastInspectionDate) : undefined,
                nextInspectionDueDate: nextInspectionDueDate ? new Date(nextInspectionDueDate).toString() : undefined,
            })
        ...
        try {
            await this.messageBroker.publishMessage(carUpdated, { data: { data: updatedCar, actor } })
        } catch (err) {
            logger.error(`Failed to publish ${carUpdated} event`, err)
        }
