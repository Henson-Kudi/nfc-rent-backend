...
import { CarCategory, CarCondition, CarDocumentType, CarInspectionStatus, CarListingType, CarStatus, FuelType, MediaType, ResponseCodes, TransmissionType } from '@/common/enums';
...
import { carCreated } from '../../../utils/messages.json'
import logger from '@/common/utils/logger';
import { Car, CarDocument, CarFeature, CarMedia, CarModel, CarOwnershipDetail, RentalPricing, User } from '@/common/entities';
import { In } from 'typeorm';

export class CreateCarUseCase implements IUseCase<[CreateCarDTO], IReturnValue<CarDto>> {
    ...
    async execute(data: CreateCarDTO): Promise<IReturnValue<CarDto>> {
        ...
        const { translations, features, model, media, owner, documents, rentalPricings, ...carData } = validData
        ...
        if (existingCar) {
            throw new AppError({
                ...
                message: existingCar?.slug === slug ? `Car with name ${enTranslation.name} already exists. Please use another name!` : `Car with vin ${carData.vin} already exists!`
            })
        }

        const savedCar = await this.carRepository.manager.transaction(async (manager) => {
            ...
            const carFeatures = features ? await featuresRepo.findBy({
                id: In(features)
            }) : []
            ...
            const car = carRepo.create({
                ...
                lastInspectionDate: carData?.lastInspectionDate ? new Date(carData.lastInspectionDate) : undefined,
                nextInspectionDueDate: carData.nextInspectionDueDate ? new Date(carData.nextInspectionDueDate) : undefined,
            ...
            if (media?.length) {
                console.log('media')
                newCar.media = await mediaRepo.save(mediaRepo.create(media.map(itm => ({
                    ...itm,
                    car: newCar
                }))))
            }
            ...
            if (rentalPricings?.length) {
                ...
                newCar.rentalPricings = await rentalPricingsRepo.save(rentalPricingsRepo.create(rentalPricings.map((itm) => ({
            ...
            if (documents?.length) {
                newCar.documents = await documentsRepo.save(documentsRepo.create(documents.map(itm => ({
                    ...itm,
                    ...
                    verificationDate: itm?.verificationDate ? new Date(itm.verificationDate) : undefined,
                    car: newCar
                }))))
            }
            ...
            if (owner) {
                ...
                newCar.ownershipDetails = [await ownershipDetailsRepo.save(ownershipDetailsRepo.create({
                    owner: ownerData,
                    ownerType: owner.ownerType,
                    percentage: owner.percentage,
                    nftId: owner.nftId,
                    acquiredDate: owner?.acquiredDate ? new Date(owner.acquiredDate) : undefined,
                    transferDate: owner?.transferDate ? new Date(owner.transferDate) : undefined,
                    status: owner?.status,
                    car: newCar
                }))]
            }
        ...
        const serialisedCar = this.serializer.serialize(CarDto, savedCar, enTranslation.locale)

        try {
            this.messageBroker.publishMessage(carCreated, { data: serialisedCar })
        } catch (error) {
            logger.error(`Failed to pubish message ${carCreated}`, error)
        }
