...
export class UpdateCarBrandUseCase
  implements IUseCase<[string, UpdateBrandDTO, User], IReturnValue<CarBrandDto>>
{
  constructor(
  ...
  async execute(
    ...
    if (enTranslation) {
      const found = await this.brandRepository.findOneBy({
        id: Not(id),
    ...
    const updatedBrand = await this.brandRepository.manager.transaction(
      async (manager) => {
        ...
        if (translations && translations?.length) {
          const existingTranslationsMap = new Map(
            foundBrand?.translations?.map((itm) => [itm.locale, itm])
          );
          ...
          const translationEntities = translations.map((trn) => {
            ...
            return existing
              ? translationRepo.merge(existing, {
                  ...trn,
                })
    ...
    try {
      this.messageBus.publishMessage(FleetEvents.brand.updated, {
        data: {
      ...
      logger.error(
        `Failed to pubish message ${FleetEvents.brand.updated}`,
        error
      );
    }
