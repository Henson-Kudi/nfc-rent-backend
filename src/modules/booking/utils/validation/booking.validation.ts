...
export const NewBookingSchema = Joi.object<CreateBookingDto>({
    ...
    pickupDate: Joi.date().iso().required().custom((value, helpers) => {
        if (!value) {
            return helpers.error("any.invalidPickupDate", { message: 'Pckup date must be in the future or today' });
        }
        const today = new Date()
        const pickup = new Date(value)

        if (!isAfter(pickup, today)) {
            return helpers.error("date.pickupInFuture", { message: "Pickup date must be in the future" });
        }
        return value;
    }).messages({
    ...
    returnDate: Joi.date().iso().required().custom((value, helpers) => {
        ...
        if (!pickupDate) {
            return helpers.error("any.invalidPickupDate", { message: "Invalid pickup date" });
        }
        ...
        if (!(differenceInDays(returnD, pickup) >= 1)) {
            return helpers.error("any.invalidDuration", { message: "Booking duration must be at least 1 day." });
        }
        return value;
    }).messages({
