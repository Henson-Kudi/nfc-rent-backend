import { SupportedCryptoCurrencies, SupportedCurrencies, SupportedFiatCurrencies } from "@/common/enums";
...
export const estimatePriceSchema = Joi.object<{
    ...
    startDate: Joi.date().iso().required().custom((val, helpers) => {
        const valid = isAfter(new Date(val), new Date())// pickup date must be in the future of now

        if (!valid) {
            return helpers.error("date.invalid", { message: "Start date must be in the future" });
        }
        // ust be in future
        return val
    }).messages({
    ...
    endDate: Joi.date().iso().required().custom((value, helpers) => {
        ...
        if (!(differenceInDays(new Date(value), new Date(startDate)) >= 1)) {
            return helpers.error("date.invalid", { message: "Booking duration must be at least 1 day" });
        }
        return value;
    }).messages({
...
export function validatePriceEstimator(data: unknown, optional: boolean = false) {
