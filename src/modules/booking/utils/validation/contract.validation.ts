...
const CreateContractViolationSchema = Joi.object<CreateContractVoilationDto>({
  ...
  evidences: Joi.array()
    .items(
      Joi.object<CreateContractVoilationDto['evidences'][0]>({
        type: Joi.string()
          .valid(MediaType.AUDIO, ...Object.values(MediaType))
          .required(),
  ...
  violationType: Joi.string().valid(
    ContractViolationType.BORDER_CROSSING,
    ...Object.values(ContractViolationType)
  ),
});
...
export const validateCreateContract = (
  ...
  NewContractSchema.validateAsync(dto, {
    abortEarly: false,
    ...(options || {}),
  });

export const validateCreateContractVoilation = (
  dto: Partial<CreateContractVoilationDto>,
  options?: AsyncValidationOptions
) =>
  CreateContractViolationSchema.validateAsync(dto, {
    abortEarly: false,
    ...(options || {}),
  });
