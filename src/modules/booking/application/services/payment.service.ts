...
export class PaymentService {
  ...
  async createPayment(booking: Booking, currency: SupportedCurrencies) {
    const isCrypto = Object.values(SupportedCryptoCurrencies).includes(
      currency as SupportedCryptoCurrencies
    );

    const selctedCurrency = [
    ...
    const product = booking?.car?.translations?.find(
      (itm) => itm.locale === 'en'
    );
    ...
    this.messageBroker.publishMessage(PaymentEvents.paymentCreated, {
      data: { payment },
    });
  ...
  async confirmPayment(paymentIntentId: string) {
    ...
    this.messageBroker.publishMessage(PaymentEvents.paymentConfirmed, {
      data: { payment },
    });
  ...
  async processRefund(paymentIntentId: string) {
    ...
    this.messageBroker.publishMessage(PaymentEvents.paymentRefunded, {
      data: { payment },
    });
  ...
  async cancelPayment(paymentIntentId: string) {
    ...
    this.messageBroker.publishMessage(PaymentEvents.paymentCancelled, {
      data: { payment },
    });
  ...
  async handleCryptoPaymentWebhookEvents(event: {
    event: string;
    data: Record<string, unknown>;
  }) {
    const paymentIntent = await this.cryptoPaymentService.retrievePayment(
    ...
    switch (event?.event) {
      ...
      case 'invoice_refundComplete':
        ...
        publishEvent = PaymentEvents.paymentRefunded;
        break;
  ...
  async handleFiatPaymentWebhookEvents(event: Stripe.Event) {
    const checkPayement = async (paymentIntent: Stripe.PaymentIntent) => {
      const payment = await this.paymentRepository.findOne({
        where: [
          { booking: { id: paymentIntent.metadata.bookingId } },
      ...
      case 'checkout.session.completed': {
        const paymentIntent =
          typeof event.data.object.payment_intent === 'string'
            ? await this.stripe.getPaymentIntent(
