...
import {
  ...
  validateCreateContractVoilation,
...
export class ContractService {
  private readonly defaultTemplatePath = 'templates/contracts/v1/default.html';
  constructor(
    @Inject()
  ...
  async createContract(payload: CreateContractDto, actor: User) {
    ...
    const contractWithBooking = await this.contractRepository.findOneBy({
      booking: { id: validPayload.bookingId },
    });

    if (contractWithBooking) {
      throw new AppError({
        message:
          'Contract with booking identifier already exists. Please update it instead',
        statusCode: ResponseCodes.BadRequest,
      });
    }

    const contract = await this.contractRepository.manager.transaction(
      async (manager) => {
        const ContractRepo = manager.getRepository(
          this.contractRepository.target
        );
        const ViolationRepo = manager.getRepository(
          this.violationsRepository.target
        );

        const totalDeductions =
          validPayload?.violations
            ?.filter((itm) => itm.isDeducted)
            ?.map(
              (item) =>
                item?.amount * item.totalUnits + (item?.processingFee || 0)
            )
            .reduce((a, b) => a + b, 0) || 0;
        const totalCharges =
          validPayload?.violations
        ...
        const contract = await ContractRepo.save(
          ContractRepo.create({
            ...
            signedAt: validPayload?.signedAt
              ? new Date(validPayload.signedAt)
              : undefined,
        ...
        const savedVoilations = validPayload?.violations?.length
          ? await ViolationRepo.save(
              validPayload?.violations?.map((violation) =>
                ViolationRepo.create({
                  ...
                  violationDate: violation?.violationDate
                    ...
                    : new Date(),
                  totalCharge:
                    violation?.amount * violation?.totalUnits +
                      (violation?.processingFee || 0) || 0,
                  contract: contract,
                })
              )
            )
  ...
  async updateContract(
    ...
    let contract = await this.contractRepository.findOne({
      where: { id },
      relations: { booking: true, violations: true },
    });
    ...
    const validPayload = await validateCreateContract(
    ...
    if (validPayload.violations?.length) {
      const updated = await this.contractRepository.manager.transaction(
        async (manager) => {
          const ContractRepo = manager.getRepository(
            this.contractRepository.target
          );
          ...
          const updatedContract = await ContractRepo.save(
            ContractRepo.merge(contract!, {
              ...rest,
              totalDeductions:
                totalDeductions || contract?.totalDeductions || 0,
      ...
      contract = await this.contractRepository.save(
        this.contractRepository.merge(contract!, validPayload)
      );
    }
  ...
  async listContracts(query: GetContractsQuery) {
    const { page: Page = 1, limit: Limit = 10, ...rest } = query;
  ...
  async addViolationToContract(
    contractId: string,
    payload: CreateContractVoilationDto,
    actor: User
  ) {
    ...
    const violation = await this.violationsRepository.manager.transaction(
      async (manager) => {
        const ViolationRepo = manager.getRepository(
          this.violationsRepository.target
        );
        ...
        const totalCharge =
          validPayload?.amount * validPayload?.totalUnits +
          (validPayload?.processingFee || 0);

        // Update contract with new violation details
        contract.totalViolationCharges =
          (contract.totalViolationCharges || 0) + totalCharge;
        contract.totalDeductions =
          (contract.totalDeductions || 0) +
          (validPayload?.isDeducted ? totalCharge : 0);
        contract.refundAmount =
          (contract.totalViolationCharges || 0) -
          (contract.totalDeductions || 0);
        await ContractRepo.save(contract);

        // Save violation
        const violation = await ViolationRepo.save(
          ViolationRepo.create({
            ...
            violationDate: validPayload?.violationDate
              ? new Date(validPayload?.violationDate)
              : new Date(),
    ...
    this.messageBroker.publishMessage(
      BookingEvents.contract.voilations.created,
      {
        data: {
          violation,
          actor,
  ...
  async updateContractVoilation(
    id: string,
    payload: Partial<CreateContractVoilationDto>,
    actor: User
  ) {
    const validPayload = await validateCreateContractVoilation(payload, {
      presence: 'optional',
    ...
    const updated = await this.violationsRepository.manager.transaction(
      async (manager) => {
        const ViolationRepo = manager.getRepository(
          this.violationsRepository.target
        );
        ...
        if (
          validPayload?.amount ||
          validPayload?.totalUnits ||
          validPayload?.processingFee
        ) {
          totalCharge =
            (validPayload?.amount || violation.amount) *
              (validPayload?.totalUnits || violation.totalUnits) +
            (validPayload?.processingFee || violation.processingFee || 0);

          const exisitingCharge =
            violation?.amount * violation?.totalUnits +
            (violation?.processingFee || 0);

          violation.contract.totalViolationCharges =
            (violation.contract.totalViolationCharges || 0) +
            totalCharge -
            exisitingCharge;
          violation.contract.totalDeductions =
            (violation.contract.totalDeductions || 0) +
            (validPayload?.isDeducted ? totalCharge - exisitingCharge : 0);

          violation.contract.refundAmount =
            (violation.contract.totalViolationCharges || 0) -
            (violation.contract.totalDeductions || 0);
        ...
        // Save violation
        const saved = await ViolationRepo.save(
          ViolationRepo.merge(violation!, {
            description:
              validPayload?.description || violation?.description || '',
            violationType:
              validPayload?.violationType || violation?.violationType,
    ...
    // Publish event to message broker
    this.messageBroker.publishMessage(
      BookingEvents.contract.voilations.updated,
      {
        data: {
          violation: updated,
          actor,
  ...
  async deleteContractVoilation(id: string, actor: User) {
    ...
    violation.contract.totalViolationCharges =
      (violation.contract.totalViolationCharges || 0) -
      (violation?.totalCharge || 0);
    violation.contract.totalDeductions =
      (violation.contract.totalDeductions || 0) -
      (violation?.isDeducted ? violation?.totalCharge || 0 : 0);
    violation.contract.refundAmount =
      (violation.contract.totalViolationCharges || 0) -
      (violation.contract.totalDeductions || 0);
    ...
    // Publish event to message broker
    this.messageBroker.publishMessage(
      BookingEvents.contract.voilations.deleted,
      {
        data: {
          violation,
          actor,
  ...
  async bulkDeleteContractVoilations(ids: string[], actor: User) {
    ...
    contract.totalDeductions =
      ...
      (violations
        ...
        .reduce((a, b) => a + b, 0) || 0);
    contract.refundAmount =
      (contract.totalViolationCharges || 0) - (contract.totalDeductions || 0);
  ...
  async generateContracPdf(identifier: string, save: boolean = false) {
    const contract = await this.contractRepository.findOne({
      where: { id: identifier },
      relations: {
        booking: {
    ...
    const pdfBuffer = await this.pdfService.generatePdfFromTemplate(
      contract?.templatePath || this.defaultTemplatePath,
      contract
    );
    ...
    // Save the PDF to s3 if required
    if (save) {
      const pdfUrl = await this.pdfService.savePdfToCloud(
        pdfBuffer,
  ...
  private setupFilter(query: GetContractsFilter) {
    ...
    if (query?.pickupDate) {
      ...
      if (end && isValid(new Date(end)) && start && isValid(new Date(start))) {
        filters.push({
          booking: { pickupDate: Between(new Date(start), new Date(end)) },
        });
      } else if (start && isValid(new Date(start))) {
        filters.push({
          booking: { pickupDate: MoreThanOrEqual(new Date(start)) },
        });
      } else if (end && isValid(new Date(end))) {
        filters.push({
          booking: { pickupDate: LessThanOrEqual(new Date(end)) },
        });
      }
    }
    if (query?.returnDate) {
      ...
      if (end && isValid(new Date(end)) && start && isValid(new Date(start))) {
        filters.push({
          booking: { returnDate: Between(new Date(start), new Date(end)) },
        });
      } else if (start && isValid(new Date(start))) {
        filters.push({
          booking: { returnDate: MoreThanOrEqual(new Date(start)) },
        });
      } else if (end && isValid(new Date(end))) {
        filters.push({
          booking: { returnDate: LessThanOrEqual(new Date(end)) },
        });
      }
    }
    ...
    if (query?.search) {
      ...
      filters.push(
        { number: ILike(`%${query.search}%`) },
        { booking: { number: ILike(`%${query.search}%`) } }
      );
    }
