...
const generateJoinLookups = (query: SelectQueryBuilder<Booking>, relations: GetBookingOptions['relations']) => {
    if (!relations) {
        return query;
    }
...
const generateGroupByFields = (relations: GetBookingOptions['relations']): string[] => {
...
export interface BookingRepository extends Repository<Booking> {
    ...
    getCarBookings(carId: string, options?: GetBookingOptions & { limit: number, skip: number }): Promise<[Booking[], number]>;

    getUserBookings(userId: string, options?: GetBookingOptions & { limit: number, skip: number }): Promise<[Booking[], number]>;
}
...
function getBookingsRepository(dataSource: DataSource) {
    return dataSource.getRepository(Booking).extend({
        ...
        getCarBookings(carId: string, options: GetBookingOptions & { limit: number, skip: number } = {
            locale: 'en',
            limit: 10,
            skip: 0,
            // sortBy: 'createdAt',
        ...
        getUserBookings(userId: string, options: GetBookingOptions & { limit: number, skip: number } = {
            locale: 'en',
            limit: 10,
            skip: 0,
            // sortBy: 'createdAt',
