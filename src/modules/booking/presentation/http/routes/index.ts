...
bookingsRouter.post('/estimate-price', requestHandler(new GetBookingPriceController()))

bookingsRouter.use(authenticateRequest())

bookingsRouter.route('/').post(requestHandler(new CreateBookingController())).get(requestHandler(new GetBookingsController())) // this get request should be used by admin site only. Need to add guard for permissions

// Webhook Routes
bookingsRouter.post('/webhooks/crypo', requestHandler(new CryptoPaymentWebhookController()))
bookingsRouter.post('/webhooks/fiat', requestHandler(new FiatPaymentWebhookController()))

bookingsRouter.route('/user').get(requestHandler(new GetUserBookingsController())) // only gets user booking
bookingsRouter.route('/car/:id').get(requestHandler(new GetCarBookingsController())) // only gets car booking. Add permissions guard here

bookingsRouter.route('/availability/:carId').get(requestHandler(new CheckCarAvailabilityController()))

bookingsRouter.route('/:id/confirm').get(requestHandler(new ConfirmBookingController()))
bookingsRouter.route('/:id/cancel').get(requestHandler(new CancelBookingController()))
