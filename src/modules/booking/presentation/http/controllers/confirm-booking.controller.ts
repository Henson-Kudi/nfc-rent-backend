...
export class ConfirmBookingController implements IController<Promise<IReturnValue<BookingDto>>> {
    handle(request: Request): Promise<IReturnValue<BookingDto>> {
        ...
        return service.confirmBooking(request.params.id, request.user)
    }
