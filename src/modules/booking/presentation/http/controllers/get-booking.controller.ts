...
export class GetBookingController implements IController<Promise<IReturnValue<BookingDto>>> {
    handle(request: Request): Promise<IReturnValue<BookingDto>> {
        ...
        return service.getBooking(request.params.id, request.query?.locale as string as SupportedLocales)
    }
