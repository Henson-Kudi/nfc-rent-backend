...
export class PermissionService {
    ...
    async checkAccess(user: User, path: string, action: ResourceAction): Promise<boolean> {
        ...
        if (user.roles.some(r => r.slug === slugify(RoleType.VIEWER))) {
            if (action !== ResourceAction.READ || path.startsWith('system')) return false;
        }

        if (user.roles.some(r => r.slug === slugify(RoleType.EDITOR))) {
            if (path.startsWith('system') && action !== ResourceAction.READ) return false;
        }
        ...
        const allPermissions = await this.permissionRepo.getUserPermissions(user.id);
    ...
    private checkWildcardAccess(permissions: string[], path: string, action: string): boolean {
        return permissions.some(permission => {
            ...
            const actionMatch = permAction === ResourceAction.MANAGE ||
                permAction === action;
    ...
    private async checkHierarchicalAccess(userId: string, path: string): Promise<boolean> {
        ...
        while (pathSegments.length > 0) {
            ...
            const hasAccess = await this.permissionRepo.userHasAccess(
                userId,
