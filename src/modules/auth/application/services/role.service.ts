...
export class RoleService {
    ...
    async createRole(name: string, permissionIdentifiers: string[]): Promise<Role> {
    ...
    async assignRole(user: User, roleName: string): Promise<User> {
        const role = await this.roleRepo.findOne({ where: { slug: slugify(roleName) } });
        if (!role) throw new AppError({ message: 'Role not found', statusCode: ResponseCodes.BadRequest });
